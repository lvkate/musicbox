...
def test_sync_returns_none_without_session():
    storage = FakeStorage(
        {"username": "x", "password": "", "user_id": 1, "nickname": "x"}
    )
    assert _sync_login_state(_api({}), storage) is None
    # Sync itself never clears; the gate does that before scanning.
    assert storage.saved == 0


def test_sync_returns_none_without_account_id():
    storage = FakeStorage(
        {"username": "", "password": "", "user_id": "", "nickname": ""}
    )
    assert _sync_login_state(_api({"profile": {"nickname": "u"}}), storage) is None
